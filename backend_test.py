...
class SpeedwayAPITester:
    def __init__(self, base_url=None):
        # Bas-URL prioritering: flagga > env > default
        self.base_url = base_url or os.getenv("API_BASE_URL", "http://127.0.0.1:8001")
        self.token = None
        self.user_id = None
        self.tests_run = 0
        self.tests_passed = 0
        ...
        # Official matches (nya endpoints)
        self.official_matches = []
        self.chosen_official_match_id = None
    ...
    def test_get_teams(self):
        ...
        if success and isinstance(response, list):
            ...
            if missing:
                print(f"   ⚠️  Missing expected teams: {missing}")
            else:
                print("   ✅ All expected Swedish teams found")
        return success
    ...
    def test_register_user(self):
        ...
        if success and 'token' in response:
            self.token = response['token']
            self.user_id = response.get('user', {}).get('id')
            print("   ✅ User registered and token obtained")
        return success

    def test_login_user(self):
        # Token verifieras via auth-skyddade anrop i andra tester.
        if not self.token:
            print("   ⚠️  No token from registration, skipping login test")
        return True
    ...
    # ---------------------------
    def test_get_matches(self):
        success, response = self.run_test("Get Matches", "GET", "/api/matches", 200)
        if success:
            print(f"   Found {len(response) if isinstance(response, list) else 0} matches")
        return success
    ...
    def test_create_match(self):
        ...
        success, response = self.run_test("Create Match", "POST", "/api/matches", 200, data=match_data, auth_required=True)
        if success and 'match_id' in response:
            self.created_match_id = response['match_id']
            print(f"   ✅ Match created with ID: {self.created_match_id}")
        return success
    ...
    def test_get_specific_match(self):
        if not self.created_match_id:
            print("   ⚠️  No match ID available, skipping specific match test")
            return True
        return self.run_test("Get Specific Match", "GET", f"/api/matches/{self.created_match_id}", 200)[0]
    ...
    def test_update_heat_result(self):
        ...
        success, match_data = self.run_test("Get Match for Heat Update", "GET", f"/api/matches/{self.created_match_id}", 200)
        if not success or not match_data.get('heats'):
            print("   ⚠️  No heats found in match")
            return False
        ...
        results = []
        positions = [1, 2, 3, 4]
        for i, (_, rider_info) in enumerate(first_heat['riders'].items()):
            results.append({
                "rider_id": rider_info['rider_id'],
                "position": positions[i],
                "status": "completed"
        ...
        result_data = {"results": results, "joker_rider_id": None, "joker_team": None}
        ok, _ = self.run_test(f"Update Heat {heat_number} Result", "PUT",
                              f"/api/matches/{self.created_match_id}/heat/{heat_number}/result",
                              200, data=result_data, auth_required=True)
        if ok:
            print(f"   ✅ Heat {heat_number} result updated successfully")
        return ok
    ...
    def test_complete_all_heats(self):
        ...
        success, match_data = self.run_test("Get Match for Heat Completion", "GET",
                                            f"/api/matches/{self.created_match_id}", 200)
        if not success or not match_data.get('heats'):
            print("   ⚠️  No heats found in match")
            return False
        ...
        for heat in match_data['heats']:
            ...
            results = []
            positions = [1, 2, 3, 4]
            for i, (_, rider_info) in enumerate(heat['riders'].items()):
                results.append({
                    "rider_id": rider_info['rider_id'],
                    "position": positions[i],
                    "status": "completed"
            ...
            result_data = {"results": results, "joker_rider_id": None, "joker_team": None}
            ok, _ = self.run_test(f"Complete Heat {heat_number}", "PUT",
                                  f"/api/matches/{self.created_match_id}/heat/{heat_number}/result",
                                  200, data=result_data, auth_required=True)
            if ok:
                completed += 1
        ...
        print(f"   ✅ Completed {completed}/15 heats")
        return completed >= 15
    ...
    def test_confirm_match(self):
        ...
        success, response = self.run_test("Confirm Match", "PUT",
                                          f"/api/matches/{self.created_match_id}/confirm", 200,
                                          auth_required=True)
        if success:
            self.user_match_id = response.get('user_match_id')
            print(f"   ✅ Match confirmed, user match ID: {self.user_match_id}")
    ...
    def test_get_user_matches(self):
        if not self.token:
            print("   ⚠️  No auth token, skipping user matches test")
            return True
        success, response = self.run_test("Get User Matches", "GET", "/api/user/matches", 200, auth_required=True)
        if success:
            count = len(response) if isinstance(response, list) else 0
            print(f"   ✅ Found {count} user matches")
            if self.user_match_id:
                found = any(m.get('id') == self.user_match_id for m in response)
                print("   ✅ Confirmed match found in user matches" if found else "   ⚠️  Confirmed match not found")
        return success
    ...
    def test_resolve_discrepancy(self):
        if not self.token or not self.user_match_id:
            print("   ⚠️  No auth token or user match ID, skipping discrepancy resolution")
            return True
        resolution_data = {"action": "accept_official"}
        return self.run_test("Resolve Discrepancy (Accept Official)", "PUT",
                             f"/api/user/matches/{self.user_match_id}/resolve", 200,
                             data=resolution_data, auth_required=True)[0]

    def test_team_colors_and_riders(self):
        if not self.created_match_id:
            print("   ⚠️  No match ID, skipping team colors test")
            return True
        success, match_data = self.run_test("Get Match for Team Colors Test", "GET",
                                            f"/api/matches/{self.created_match_id}", 200)
        if not success or not match_data.get('heats'):
            return False
        first_heat = match_data['heats'][0]
        home_colors_found, away_colors_found = [], []
        for _, rider_info in first_heat['riders'].items():
            if rider_info.get('team') == 'home':
                home_colors_found.append(rider_info.get('helmet_color'))
            else:
                away_colors_found.append(rider_info.get('helmet_color'))
        expected_home = ['#DC2626', '#1D4ED8']
        expected_away = ['#EAB308', '#FFFFFF']
        ok = all(c in expected_home for c in home_colors_found) and all(c in expected_away for c in away_colors_found)
        if ok:
            print(f"   ✅ Team colors OK - Home: {home_colors_found}, Away: {away_colors_found}")
        else:
            print(f"   ❌ Team colors incorrect - Home: {home_colors_found}, Away: {away_colors_found}")
        return ok
    ...
    def test_get_specific_team(self):
        if not self.team_ids:
            print("   ⚠️  No team IDs available, skipping specific team test")
            return True
        return self.run_test("Get Specific Team", "GET", f"/api/teams/{self.team_ids[0]}", 200)[0]

    # ---------------------------
    # OFFICIAL MATCHES (nya)
    # ---------------------------
    def test_get_official_matches(self):
        """GET /api/official-matches (kräver auth)"""
        if not self.token:
            print("   ⚠️  No auth token, skipping official matches")
            return False
        ...
        if success and isinstance(response, list):
            self.official_matches = response
            print(f"   ✅ Official matches found: {len(self.official_matches)}")
            if self.official_matches:
                # Välj första matchen som inte är 'used'
                self.chosen_official_match_id = self.official_matches[0].get("id")
                print(f"   Using official match id: {self.chosen_official_match_id}")
            else:
                print("   ⚠️  No official matches available (empty list).")
        return success

    def test_mark_official_match_as_used(self):
        """PUT /api/official-matches/{match_id}/mark-used (kräver auth)"""
        if not self.token:
            print("   ⚠️  No auth token, skipping mark-used")
            return True  # Inte ett hårt fel för hela testsviten

        if not self.chosen_official_match_id:
            print("   ⚠️  No official match id available (list was empty). Skipping mark-used.")
            return True  # Hoppa över om inget att markera

        return self.run_test(
            "Mark Official Match As Used",
            "PUT",
            f"/api/official-matches/{self.chosen_official_match_id}/mark-used",
            200,
            auth_required=True
        )[0]
    ...
    def run_all_tests(self):
        ...
        # OFFICIAL MATCHES
        self.test_get_official_matches()
        self.test_mark_official_match_as_used()
...
def main():
    parser = argparse.ArgumentParser(description="Run Speedway Elitserien API tests")
    parser.add_argument("--base", type=str, help="Base URL of the API, e.g. http://127.0.0.1:8001")
    args = parser.parse_args()

    tester = SpeedwayAPITester(base_url=args.base)
    success = tester.run_all_tests()
    return 0 if success else 1
